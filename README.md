...
> **📌 重要提示**：
>
> - 首次部署会自动 fork 项目到你的 GitHub 账号
> - 需要授权 Cloudflare Workers 访问你的 GitHub
> - 部署完成后仍需手动配置环境变量和 D1 数据库（见下方步骤 4-6）
...
- 使用本地 D1 数据库（`.wrangler/state` 目录）
- 支持热重载，修改代码自动刷新
- 所有配置与生产环境一致（从 `wrangler.toml` 读取）
